...
def visualize(prev_container, curr_container, focal, pp, prev_frame_id, curr_frame_id):
    ...
    print(np.array([norm_foe]), focal, pp)
...
def run_sfm(curr_id, prev_id, frame_list, pkl_path, curr_x_red, curr_y_red, curr_x_green, curr_y_green, prev_x_red,
            prev_y_red, prev_x_green, prev_y_green):
    prev_container = FrameContainer(frame_list[prev_id])
    curr_container = FrameContainer(frame_list[curr_id])
    with open(pkl_path, 'rb') as pklfile:
        data = pickle.load(pklfile, encoding='latin1')
    focal = data['flx']
    pp = data['principle_point']
    prev_container.traffic_light = np.array([[prev_x_red[0], prev_y_red[0]]])
    for i in range(len(prev_x_red)):
        prev_container.traffic_light = np.concatenate(
            (prev_container.traffic_light, np.array([[prev_x_red[i], prev_y_red[i]]])))
    for i in range(len(prev_x_green)):
        prev_container.traffic_light = np.concatenate(
            (prev_container.traffic_light, np.array([[prev_x_green[i], prev_y_green[i]]])))
    curr_container.traffic_light = np.array([[curr_x_red[0], curr_y_red[0]]])
    for i in range(len(curr_x_red)):
        curr_container.traffic_light = np.concatenate((curr_container.traffic_light, [[curr_x_red[i], curr_y_red[i]]]))
    for i in range(len(curr_x_green)):
        curr_container.traffic_light = np.concatenate(
            (curr_container.traffic_light, [[curr_x_green[i], curr_y_green[i]]]))
    EM = np.eye(4)
    for i in range(prev_id, curr_id):
        EM = np.dot(data['egomotion_' + str(i) + '-' + str(i + 1)], EM)
    curr_container.EM = EM
    curr_container = SFM.calc_TFL_dist(prev_container, curr_container, focal, pp)
    visualize(prev_container, curr_container, focal, pp, frame_list[prev_id], frame_list[curr_id])